...
class DialogNewNextActionFragment : DialogFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        val action:String = arguments?.get("currentAction") as String
        dialog_text_action_item.text = action
        dialog_button_create_next_action.setOnClickListener {
            val nextActionName = dialog_edit_text_next_action.text.toString()
            actionViewModel.moveToNextAction(nextAction = nextActionName, action = action)
            dialog?.dismiss()
